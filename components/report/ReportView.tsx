...
export function ReportView({ report, assessment, assessmentId }: ReportViewProps) {
  ...
  async function downloadPDF() {
    ...
    try {
      ...
      a.href = url;
      a.download = `suverankoll-${(assessment.org_name ?? 'report').replace(/\s+/g, '-').toLowerCase()}.pdf`;
      a.click();
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white">
            {assessment.org_name ?? 'Your organization'}
          </h1>
          <p className="text-zinc-500 text-sm mt-1">
            {assessment.sector} · {assessment.org_size} employees ·{' '}
      ...
      <section className="mb-8">
        <h2 className="text-lg font-bold text-white mb-4">Risk Analysis by Category</h2>
      ...
      <div className="mt-8 p-4 bg-zinc-900 border border-zinc-800 rounded-lg">
        <p className="text-xs text-zinc-500 text-center">
          Report generated by SuveranKoll — an IT-Bladet tool. Analysis is based on
          self-assessment data and should be supplemented with a professional security review.
        </p>
      </div>
    </div>
  );
